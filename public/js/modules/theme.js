// ─── THEME ───────────────────────────────────────────────
// Kelola dark/light mode dan sinkronisasi ikon di semua tempat.

const moonSVG = `<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>`;
const sunSVG  = `<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="5"/><line x1="12" y1="1" x2="12" y2="3"/><line x1="12" y1="21" x2="12" y2="23"/><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"/><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"/><line x1="1" y1="12" x2="3" y2="12"/><line x1="21" y1="12" x2="23" y2="12"/><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"/><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"/></svg>`;

const moonSVGSmall = moonSVG.replace('22" height="22"', '20" height="20"');
const sunSVGSmall  = sunSVG.replace('22" height="22"', '20" height="20"');

// Sinkronkan semua ikon & label tema
function updateThemeUI(isLight) {
  const pairs = [
    ['sidebarThemeIcon',  'sidebarThemeLabel',  false],
    ['settingsThemeIcon', 'settingsThemeLabel', false],
    ['navbarThemeIcon',   'navbarThemeLabel',   false],
  ];

  pairs.forEach(([iconId, labelId]) => {
    const iconEl  = document.getElementById(iconId);
    const labelEl = document.getElementById(labelId);
    if (iconEl)  iconEl.innerHTML  = isLight ? sunSVGSmall  : moonSVGSmall;
    if (labelEl) labelEl.textContent = isLight ? 'Mode Terang' : 'Mode Gelap';
  });
}

function toggleTheme() {
  const isLight = document.documentElement.getAttribute('data-theme') === 'light';
  if (isLight) {
    document.documentElement.removeAttribute('data-theme');
    localStorage.setItem('theme', 'dark');
    updateThemeUI(false);
  } else {
    document.documentElement.setAttribute('data-theme', 'light');
    localStorage.setItem('theme', 'light');
    updateThemeUI(true);
  }
}

// Terapkan tema tersimpan sebelum halaman render (cegah flash)
(function applyStoredTheme() {
  if (localStorage.getItem('theme') === 'light') {
    document.documentElement.setAttribute('data-theme', 'light');
  }
})();

// ── Dropdown pengaturan (sidebar) ────────────────────────
function toggleSettingsDropdown() {
  document.getElementById('settingsDropdown')?.classList.toggle('open');
}

document.addEventListener('click', (e) => {
  const wrap = document.querySelector('.sidebar-settings-wrap');
  if (wrap && !wrap.contains(e.target)) {
    document.getElementById('settingsDropdown')?.classList.remove('open');
  }
});

// ── Dropdown pengaturan (navbar mobile) ──────────────────
function toggleNavbarSettings() {
  document.getElementById('navbarSettingsDropdown')?.classList.toggle('open');
}

function closeNavbarSettings() {
  document.getElementById('navbarSettingsDropdown')?.classList.remove('open');
}

document.addEventListener('click', (e) => {
  const wrap = document.querySelector('.navbar-settings-wrap');
  if (wrap && !wrap.contains(e.target)) closeNavbarSettings();
});
